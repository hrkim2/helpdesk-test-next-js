...
export default function CreateQna({data, page}) {
    ...
    function submitData(update){
        ...
        if(title.length*detail.length>0){
            if(!update){
                ...
                axios.put(`http://localhost:1337/api/qnas/${data.id}`,{
                    "data": {
                        "boardCd": qna.boardCd,
                        "qId": qna.qId,
                        "qTitle": title,
    ...
    return <div>
            ...
            <thead>
                <tr key={'1'}>
                    ...
                    <td key={'2'}>
                        ...
                        :<input
                            type='text' id='title' name='qTitle'
                            value={qna.qTitle}
            ...
            <tbody>
                ...
                <tr key={'4'}>
                    <td key={'6'} colSpan={2}>
                        ...
                        :<textarea
                            id='detail' name='qDetail' cols={50} rows={25}
                            value={qna.qDetail}
                            onChange={e=>updateInputform(e)}
                        />
                        }
        ...
        <Button
            ...
            onClickBtn={()=>submitData(qna?true:false)}
        />
        <style jsx>{`
            table{
                width:100%;
...
export async function getServerSideProps({query}){
    ...
    if(openName==='udt'){
        if(id){
            const {data} = await(
                await fetch(`http://localhost:3000/qnas/${id}`)
            ).json();
